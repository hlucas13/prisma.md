...
import { clearHistory, formatHistoryDate, getHistory, saveSnapshot } from './history-store';
...
function applyLanguage(lang) {
  ...
  if (langContainer) {
    langContainer
      ...
      .forEach((el: Element) =>
        el.classList.toggle('selected', (el as HTMLElement).dataset.lang === lang),
      );
...
document.addEventListener('click', (e) => {
  ...
  if (
    ...
    !(target as HTMLElement).closest('#btn-export, #btn-settings, #btn-themes, #btn-hamburger')
  ) {
    closeAllMenus();
  }
});
...
document.addEventListener('mousemove', (e) => {
  ...
  (document.querySelector('.editor-pane') as HTMLElement).style.flex = `0 0 ${pct}%`;
  (document.querySelector('.preview-pane') as HTMLElement).style.flex = `0 0 ${100 - pct}%`;
  cm.refresh();
});
...
btnCopyTeams.addEventListener('click', async () => {
  ...
  const plain = (preview.querySelector('.markdown-body') as HTMLElement).innerText;
  try {
    await navigator.clipboard.write([
      new ClipboardItem({
        'text/html': new Blob([html], { type: 'text/html' }),
...
editorPane.addEventListener('drop', (e) => {
  ...
  const isMd =
    nameLower.endsWith('.md') || nameLower.endsWith('.markdown') || file.type === 'text/markdown';
...
const portraitQuery = window.matchMedia('(max-width: 768px), (orientation: portrait)');
...
paneHeaderPreview.addEventListener('click', () => togglePane(previewPaneEl, paneHeaderPreview));
